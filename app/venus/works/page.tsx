...
export default function AdminWorksPage() {
  ...
  const fetchWorks = async () => {
    setLoading(true)
    try {
      const response = await fetch('/.netlify/functions/works');
    ...
    startTransition(async () => {
      try {
        ...
        if (fileToUpload.file) {
          const presignedUrlResponse = await fetch('/.netlify/functions/works', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
            },
            body: JSON.stringify({
              type: 'getSignedUploadUrl',
              fileName: fileToUpload.file.name,
              contentType: fileToUpload.file.type,
              bucketName: 'works-images',
            }),
          });
          const presignedUrlResult = await presignedUrlResponse.json();
        ...
        let result;
        if (editingWork) {
          formData.set("id", editingWork.id);
          const response = await fetch('/.netlify/functions/works', {
            method: 'POST',
            body: formData,
          });
    ...
    startTransition(async () => {
      try {
        const response = await fetch('/.netlify/functions/works', {
          ...
          body: JSON.stringify({ type: 'deleteWork', id, imageUrl }),
        });
        const result = await response.json();
        if (result.success) {
          toast({
